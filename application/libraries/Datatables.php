<?php if(!defined('BASEPATH')) exit('No direct script access allowed');
  ...
  */
  class Datatables
  {
    ...
    * @return string
    */
    public function generate($output = 'json', $charset = 'UTF-8', $type = null)
    {
      ...
      $this->get_ordering();
      $this->get_filtering();
      return $this->produce_output(strtolower($output), strtolower($charset),strtolower($type));
    }
    ...
    * @return mixed
    */
    private function produce_output($output, $charset, $type = null)
    {
      $aaData = array();
      ...
      if($type == 'assoc'){
        ...
        $add = $this->add_columns;
        $new_column = array();
        foreach ($res as $row_key => $row_val) {
          foreach ($this->add_columns as $field => $val) {
              $new_column[$field] = $this->exec_replace($val,$row_key);         
              
              $aaData[$row_key] = array_merge_recursive($row_val,$new_column);
            }  
        }
      }else{
